...
#include <mangrove/expression_syntax.hpp>
...
template <typename Base, typename T, size_t N, size_t M, T Base::*t>
    constexpr std::enable_if_t < N<M && hasField<Base, T, N, M>::value, bool> wrapbool() {
    if (std::get<N>(Base::mangrove_mapped_fields()).t == t) {
        return true;
    } else {
...
constexpr std::enable_if_t<(N < M) && hasField<Base, T, N, M>::value, const nvp<Base, T>> wrapimpl(
    T Base::*t) {
    if (std::get<N>(Base::mangrove_mapped_fields()).t == t) {
        return std::get<N>(Base::mangrove_mapped_fields());
    } else {
...
constexpr const nvp<Base, T> wrap(T Base::*t) {
    return wrapimpl<Base, T, 0, std::tuple_size<decltype(Base::mangrove_mapped_fields())>::value>(
        t);
}
...
struct hasCallIfFieldIsPresent<
    ...
    std::enable_if_t<wrapbool<
        Base, T, 0, std::tuple_size<decltype(Base::mangrove_mapped_fields())>::value, ptr>()>> {
