...
struct DataC : public boson::UnderlyingBSONDataBase {
...
TEST_CASE(
    ...
    boson::BSONOutputArchive archive(os);
...
struct DataD : public boson::UnderlyingBSONDataBase {
...
TEST_CASE(
...

    {
        ...
        boson::BSONInputArchive archive(is);
        archive(CEREAL_NVP(tp), CEREAL_NVP(test_obj_arr), CEREAL_NVP(i), a1, a2, b1, c1);
...
TEST_CASE("the BSON archiver successfully serializes embedded classes with dot notation") {
    boson::bson_ostream os([](bsoncxx::document::value v) { REQUIRE(countKeys(v.view()) == 8); });

    boson::BSONOutputArchive archive(os, true);
